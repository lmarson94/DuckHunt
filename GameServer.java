...
class GameServer
{
    ...
    private class SPlayer
    {
        ...
        SPlayer(BufferedReader pInputStream, PrintStream pOutputStream, int pID) //struct dove registrare informazioni sul giocatore
        {
            mInputStream = pInputStream; //pipe input
            mOutputStream = pOutputStream; //pipe output
            mID = pID;
            mNumSent = 0;
            mScore = 0;
            mGameOver = false;
    ...
    private class BirdSequence //struct che rappresenta singolo uccello, composta da specie e movimenti
    {
        public int mSpecies;
        public int[] mActions;
    }
    ...
    public void load(Readable pStream) //carica sul server l'environment
    {
        // Parse the environment file
        Scanner lScanner = new Scanner(pStream);
        mMaxRounds = lScanner.nextInt();
        mEnvironment = new BirdSequence[mMaxRounds][]; //matrice, righe = rounds and colonne = ogni singolo uccello del round
        for (int r = 0; r < mMaxRounds; ++r)
        {
            ...
            for (int b = 0; b < lNumBirds; ++b)
            {
                ...
                // Read species
                mEnvironment[r][b].mSpecies = lScanner.nextInt(); //primo numero per sequenza = specie dell'uccello
    ...
    public void run()
    {
        ...
        for (int r = 0; r < mEnvironment.length; ++r)
        {
            ...
            {
                // Let the birds fly
                for (int b = 0; b < mEnvironment[r].length; ++b) //per ogni uccello
                    mBirds[b].addObservation(mEnvironment[r][b].mActions[i]); //carica su ogni uccello la prossima mossa
            ...
            // Send any trailing observations, per migliorare dati per il guess
            for (SPlayer lPlayer : mPlayers)
                sendBirds(lPlayer);
    ...
    private void playerShoot(SPlayer pPlayer)
    {
        ...
        Deadline lDue = new Deadline(mTimeForShoot); //praticamente è un timer
        ...
        try
        {
            if ((lString = pPlayer.mInputStream.readLine()) == null) //controlla che il client risponda
            {
        ...
        if (lDue.remainingMs() < 0) //controlla che il client abbia risposto in tempo
        {
        ...
        if (lBird >= 0 && lBird < mBirds.length)
        {
            ...
            if (lMovement == lTrueMovement && lMovement != Constants.MOVE_DEAD)
            {
                ...
                pPlayer.mOutputStream.println("HIT " + lBird + " " + mTimeForHit); //forse un po di tempo in più per aggiornare il modello
            }
            else
            {
                pPlayer.mScore -= 1;
    ...
    private void playerGuess(SPlayer pPlayer)
    {
        ...
        Deadline lDue = new Deadline(mTimeForGuess); //setta il timer
        ...
        try
        {
            if ((lString = pPlayer.mInputStream.readLine()) == null) //controlla che la pipe non sia nulla
            {
                System.err.println("getline failed for player " + pPlayer.mID);
                pPlayer.mGameOver = true;
                return;
        ...
        if (lDue.remainingMs() < 0) //se c'è una risposta ma il tempo è scaduto
        {
            System.err.println("Player " + pPlayer.mID + " timed out");
            removePlayer(pPlayer, "TIMEOUT");
            return;
        ...
        StringTokenizer lIn = new StringTokenizer(lString);
        int lScore = 0;
        int[] lRevealing = new int[mBirds.length]; //vettore con le guess per le specie
        Arrays.fill(lRevealing, Constants.SPECIES_UNKNOWN); //inizializzato ad unknown
        ...
        for (int i = 0; i < mBirds.length; ++i)
        {
            ...
            if (lGuessedSpecies == Constants.SPECIES_UNKNOWN) //se il client non vuole indovinare
                continue;
        ...
        if (lIn.hasMoreTokens()) //se è rimasta roba sul tokenizer -> hai sbagliato
        {
        ...
        pPlayer.mScore += lScore; //cambia punteggio in base hai guess effettuati
